...
static sync_test_atomic_t sync_owned_bytes_channel_test_next_init_error = 0;
static sync_test_atomic_t sync_owned_bytes_channel_test_next_operation_error = 0;

static int32_t sync_owned_bytes_channel_test_take_next_operation_error(void) {
  return sync_test_atomic_take(
    &sync_owned_bytes_channel_test_next_operation_error
  );
}
...
MOONBIT_FFI_EXPORT sync_owned_bytes_channel_handle_t *sync_owned_bytes_channel_new(
  ...
  *status = sync_test_atomic_take(&sync_owned_bytes_channel_test_next_init_error);
  if (*status == 0) {
...
static int32_t sync_owned_bytes_submit(
  ...
  *error = sync_owned_bytes_channel_test_take_next_operation_error();
  if (*error != 0) {
    return 2;
  }
...
MOONBIT_FFI_EXPORT void sync_owned_bytes_channel_test_fail_next_init(
  int32_t status
) {
  sync_test_atomic_store(
    &sync_owned_bytes_channel_test_next_init_error,
    status
  );
}

MOONBIT_FFI_EXPORT void sync_owned_bytes_channel_test_fail_next_operation(
  int32_t status
) {
  sync_test_atomic_store(
    &sync_owned_bytes_channel_test_next_operation_error,
